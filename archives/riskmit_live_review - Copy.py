...
# ==================================================================================
    ...
    # COLUMN HEADINGS
        ...
        for column in tree_balances['columns']:
            ...
            tree_balances.column(column, width=90)
    # PRINT BALANCE ROWS
    ...
    '''# TOTAL HEADINGS
        ...
        for column in tree_balances['columns']:
            ...
            tree_totals.column(column, width=30)
    # PRINT TOTAL ROWS
...
# ==================================================================================
    def draw_positions(e):
        ...
        df_pos = positions(q,account_number)
        df_pos_summary = df_pos
        df_pos_summary = df_pos_summary.drop(['symbolId','closed_qty','pnl_day','pnl_closed','isRealTime','isUnderReorg'], axis=1)
    # COLUMN HEADINGS
        tree_positions['columns'] = list(df_pos_summary.columns)
    ...
    def position_detail_window():
            ...
            w3.title('POSITIONS')
        # FRAMES
            frame_data = LabelFrame(w3, text="CURRENT POSITIONS")
            frame_data.place(x=5, y=5, height=500, width=1390) # set the height and width of Jake is a Bad Dog
            data_scroll = Scrollbar(frame_data) # add scroll bar
            data_scroll.pack(side=RIGHT, fill=Y)
        # TREE
            data_tree = ttk.Treeview(frame_data, height= 600)
            data_tree.pack()
        ...
        # CLOSE BUTTON
            ...
            df_pos_detail = positions(q,account_number)
        # COLUMN HEADINGSs
            data_tree['columns'] = list(df_pos_detail.columns)
...
# ==================================================================================
    ...
    def order_detail_window():
            ...
            w4.title('ORDERS')
        # FRAMES
            frame_ord_detail = LabelFrame(w4, text="OPEN ORDERS")
            frame_ord_detail.place(x=5, y=5, height=500, width=1390) # set the height and width of data_scroll = Scrollbar(frame_data) # add scroll bar
        # SCROLL
            scroll_orders_all_y = Scrollbar(frame_ord_detail)
            scroll_orders_all_y.pack(side=RIGHT, fill=Y)
            # HORIZONTAL SCROOL
            scroll_orders_all_x = Scrollbar(frame_ord_detail, orient='horizontal')
            scroll_orders_all_x.pack(side=BOTTOM, fill=X)
        # TREE
            tree_orders_all = ttk.Treeview(frame_ord_detail,height=600,yscrollcommand=scroll_orders_all_y, selectmode='extended', xscrollcommand=scroll_orders_all_x)
        ...
        # CLOSE BUTTON
            ...
            close_button.pack()
            df_orders = create_orders_df(q,account_number)[2]
        # COLUMN HEADINGSs
